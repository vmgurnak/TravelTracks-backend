...
export const camperAddShema = Joi.object({
  ...
  form: Joi.string()
    ...
    .required()
    .messages({ 'any.required': 'form is required' }),
  ...
  reviews: Joi.array().items(
    Joi.object({
      ...
      reviewer_rating: Joi.number().min(1).max(5),
      comment: Joi.string(),
    }),
  ),
});
...
export const camperUpdateShema = Joi.object({
  ...
  form: Joi.string().valid(...TravelTracksForm),
  length: Joi.string(),
  width: Joi.string(),
  height: Joi.string(),
  tank: Joi.string(),
  consumption: Joi.string(),
  transmission: Joi.string().valid(...TravelTracksTransmission),
  engine: Joi.string().valid(...TravelTracksEngine),
  AC: Joi.boolean(),
